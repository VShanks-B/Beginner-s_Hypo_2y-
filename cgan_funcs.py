...
def loss_function(recon_x, x, cond_data, wx, wy, fun_list):
...
#         recon_cond_data = recon_cond_data.to(torch.device('mps'))
    ...
    total_loss = (wx * x_loss + wy * y_loss).mean()
...
def train_cgan(cgan, train_loader, optimizer, beta, wx, wy, epoch, fun_list, step_to_print=1):
    cgan.train()
    train_loss = 0.0
    recon_loss = 0.0
    cond_loss = 0.0
...
#         if torch.cuda.is_available():
        ...
        # ===================forward=====================
        recon_data, z_mean, z_logvar = cgan(data, cond_data)
        loss, loss_x, loss_y = loss_function(recon_data, data, cond_data, wx, wy,
                                                       fun_list)
    ...
    if epoch % step_to_print == 0:
        print('Train Epoch {}: Average Loss: {:.6f}, x_loss: {:3f}, y_loss: {:3f}'.format(epoch, train_loss,
...
def test_cgan(cgan, test_loader, beta, wx, wy,fun_list):
    cgan.eval()
    test_loss = 0.0
...
#                 data = data.to(torch.device('mps'))
            ...
            recon_data = cgan(data, cond_data)

            loss,_,_ = loss_function(recon_data, data, cond_data, wx, wy, fun_list)
...
def generate_samples(cgan, num_samples, given_y, input_shape, zmult = 1):
    ...
    with torch.no_grad():
        for _ in range(num_samples):
            ...
            sample = cgan.generator(z, given_y)
